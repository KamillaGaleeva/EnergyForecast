...
def read_consumption_data(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    data = db.query(models.ConsumptionData).order_by(models.ConsumptionData.timestamp.desc()).offset(skip).limit(limit).all()
    return data
...
def predict_endpoint(
    ...
    try:
        ...
        from .ml_model import save_prediction_to_db
